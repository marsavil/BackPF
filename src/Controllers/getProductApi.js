...
const getProducts = async () => {
  ...
  let productsMotorola = await axios(
  ...
  let productsSamsung = await axios(
  ...
  let productsHuawei = await axios(
    "https://api.mercadolibre.com/sites/MLA/search?q=Huawei"
  );
  productsHuawei = productsHuawei.data.results.slice(0, 25);
  const cellphonesHuawei = productsHuawei.map((e) => {
    let img = e.thumbnail;
  ...
  let productsIphone = await axios(
